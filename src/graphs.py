...
interactions = load_data("dataset/RAW_interactions.csv.zip")
# interactions = load_data("dataset/PP_recipes.csv.zip")  # Alternative dataset loading (commented out)
...
fig1 = px.pie(
    ...
    color_discrete_sequence=px.colors.sequential.Blues[::-1]  # Shades from light to dark blue
)
...
fig2.add_annotation(
    text="Instagram",
    x='2011-10-31',  # x position in the time interval (adjust as needed)
    y=6000,  # y position (80% of the curve height)
    showarrow=False,
...
fig3.update_layout(
    # xaxis_title="All the recipes stored in the data",  # x-axis label (commented out)
